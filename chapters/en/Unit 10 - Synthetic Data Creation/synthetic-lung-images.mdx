...
We can see that some of the synthetic lung images look good, but some are hazy. Researchers generating synthetic medical images typically employee a "human in the middle"--in this case, a expert radiologist--to evaluate the synthetic images. Then, only the images that fool the expert are included with the real data to train the model.

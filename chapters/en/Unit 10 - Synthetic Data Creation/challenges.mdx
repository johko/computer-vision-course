...
Before using synthetic data to train a model, however, there are several important things that should be considered.
...
or may be different in the real world. A good practice is to evaluate your dataset using a measure such as Frechet Inception Distance (FID), IS, or the Classification
Accuracy Score (CAS).

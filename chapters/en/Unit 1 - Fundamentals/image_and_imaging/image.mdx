...
It might be weird that we are going to explain to you what an image is in a Computer Vision Course. Presumally, the reason that you got here in the first place was because you wanted to know more about how to process image and video format. It seems trivial, but you are in here for a surprise! When it comes to images, there are much more than it meets the eye (pun-intended).

## Definition of Image

An image is a visual representation of an object, a scence, a person, or even a concept They can be photographs, paintings, drawings, schemes, scans and problably a thing or more. One the more suprising things is that an image is also a function. More precisely, an image is an n-dimensional function. We will first consider it to be two-dimensional $(n=2)$. We can denote: $F(X, Y)$ , where $X$ and $Y$ are spatial coordinates. Do not get distracted by the fancy name. Spatial coordinates are just the system that we use to describe the positions of objects in a physical space with the most common one being the $2D$ cartesian system.The amplitude of $F$ at a pair of coordinates $(x_i, y_i)$ is the intensity or gray level of the image at that point. The intensity is what gives you the perception of light and dark. Typically, when we have the pair coordinate $x_1$ and $y_1$, we refer to them as pixels (picture elements).
...
That is not the only way one can create an image.  Sometimes, they are created by computers with or without the help of AI.  We have dedicated chapter for images that do have a little helping hand from AI. Most of the terminology, we will introduce here will still be apply

A different type of image is volumetric or $3D$ images. The number of dimensions in $3D$ images is equal to three. As a result, we have a $F(X, Y, Z)$ function. Most of our reasoning still applies, with the only difference being that the triplet $x_i, y_i, z_i$ is called a voxel (volume element). These images can be acquired in $3D$, that is, the images are acquired in a way that is reconstructed in a $3D$ space. Examples of such images include medical scans, magnetic resonance, and certain types of microscopes. It is also possible to reconstruct a $3D$ image from a $2D$ one. Reconstructing is a challenging task and it has also its dedicated chapter.

Now that we have discussed space, we can talk about color. The good news is you have likely heard of image channels too. You might not understand what they mean but fear not! Image channels are simply the different color components that make an image. In reference to the $F(X,Y)$, this means we will have $F$ for each color component. Each color has its own intensity level. For a channel that picks up the color red, a high intesity means that the color is very red and a low intensity means that there is little to no red in there.

The most common color model is RGB (Red, Green, Blue). Three colors, three channels, three $F(X, Y)$. In this model, the true color of a coordinate $(x_i, y_i)$ is given by the combination of the three different F $(x_i, y_i)$ at that location. Combined, these three colors describe a wide spectrum of colors.
...
There are special types of images where the coordinates $x_i, y_i$ do not describe an intensity value, but instead label a pixel. The simplest example of an operation that results in such an image is separating foreground and background. Everything that is foreground receives the label $1$, and everything that is background receives the label $0$. These images are commonly referred to as labeled images. When the are only two labels, like our example, we call them binary images or masks. 
...
Alternatively, images can be represented as graphs where each node is a coordinate, and the edges are the neighboring coordinates. Take a moment to let that sink in. It also means that the algorithms and models that are used for graphs can also be used for images! The inverse can also be true - you may be able to transform a graph into an image and analyze it as if it were a picture.

So far, we proposed a rather flexible definition of what an image is. This definition can accommodate different ways of acquiring visual data, but they all highlight the same crucial aspect: images are data points that contain a lot of spatial information. The key differences are the spatial resolution (either 2D or 3D), their color systems (RGB or others), and whether they have a time component attached to them.

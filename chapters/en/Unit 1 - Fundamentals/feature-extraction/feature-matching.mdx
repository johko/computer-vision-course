...
```
6. Clean up the correspondences using Random Sample Consensus (RANSAC),which helps us
to deal with noise or outliers in the data.
...
Corrrect matches are visualized in green, while impressize matches are in blue.

...
_First install and load libraries_
...
_Brute Force with ORB (binary) descriptors_
...
3.  Indicate whether the image is an "indoor" or "outdoor" image

```python
matcher = LoFTR(pretrained="indoor")
```

4.  LoFTR only works on grayscale images, so convert to grayscale
...
6. We can clean up the correspondences using Random Sample Consensus (RANSAC), which help
...
7. Finally, we visualize the matches

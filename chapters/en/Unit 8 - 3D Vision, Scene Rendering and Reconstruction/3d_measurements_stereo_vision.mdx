...
4. Internal parameters of the camera such as focal length and location of principal point. The principal point is the point where the optical axis intersects the image plane. It's location in the image plane is usually denoted as (Ox,Oy).
...
If both images (with original and new camera positions) contain point P, then we can conclude that the 3D lines must intersect at least once, and that intersection point is point P.Furthermore, we can envision an infinite number of points where both lines intersect only if the two lines are collinear, which is achievable if the pinhole at the new camera position lies somewhere on the original 3D line. For all other positions and orientations of the new camera location, the two 3D lines must intersect at precisely one point, which is the location where point P lies. 
...
* O_x and O_y refer to pixel coordinates of the principal point 
...
* Intrinsic camera parameters like focal length and location of principal point are known for an OAK-D Lite since the device comes pre-calibrated and these parameters can be read from the device using it's python API. For a smartphone camera, intrinsic parameters need to be determined and could be found by performing camera calibration or sometimes present in the metadata of the image captured using the smartphone. 

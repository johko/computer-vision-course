...
Furthermore it allows us to store large scenes with a smaller memory footprint than explicit reresentation, since we merely need to store the weights of our neural network compared to voxel grids, that increase in memory size by a cubic term.
...
Since 2020, more than 620 preprints and publications have been released with more than 250 source codes on GitHub. 
*(as of Dec 2023, statistics from [paperswithcode.com](https://paperswithcode.com/method/nerf))*

Since the first formulation of NeRFs requires long training times (up to days on beefy GPUs) there have been a lot of advancements towards fatser training and inference. 
An important leap would be NVIDIA's [Instant-ngp](https://nvlabs.github.io/instant-ngp/), that introduces innovative techniques that efficiently encode input data. 
This allowed the authors to shrink the MLP down significantly, making it faster to train and query while performing on par quality wise with then state-of-the-art methods. 
[Mipnerf-360](https://jonbarron.info/mipnerf360/) is also worth mentioning since it intoduced a scene contraction that allows us to represent scenes that are unbounded in all directions, which is important for real world applications. 
[Zip-NeRF](https://jonbarron.info/zipnerf/) which combines recent advancements like the encoding from [Instant-ngp](https://nvlabs.github.io/instant-ngp/) and the scene contraction from [Mipnerf-360](https://jonbarron.info/mipnerf360/) to handle real world situation whilst decreasing training times to under an hour. 
*(this is still measured on beefy GPUs to be fair)*

Since the field of NeRFs is rapidly evolving, I am adding a section **Future Direction** on the bottom where I will tease the latest reseaerch and the possible future direction of NeRFs.
...
The explanations above are important to understand the basic cocepts and similar if not identical in every NeRF model. 
...
The second trick worth mentioning is that most approaches use smart approaches to sample points in space. 
...
I believe that we will soon be able to stream a real world scene in real time on an edge device and this is a huge leap towards a realistic *Metaverse*. 

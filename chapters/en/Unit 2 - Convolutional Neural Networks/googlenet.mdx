...
#### 2.1 The Inception "Network In Network" Module 
...
We want to convert a feature map of $S \times S \times 128$ to $S \times S \times 256$ via a 5x5 convolution. The number of parameters(excluding biases) is 5x5x128x256 = 819,200. However, if we reduce the feature dimension first by a $1 \times 1$ convolution to 64, then the number of parameters(excluding biases) is $1\times 1\times 128\times 64 + 5\times 5\times 64\times 256 = 8,192 + 409,600 = 417,792$. The parameters were reduced by almost half.
...
<!-- TODO: <<<Attach inception with dimensionality reduction>>> -->
Figure 2

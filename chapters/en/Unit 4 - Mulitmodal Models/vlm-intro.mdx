...
We think of modality, initially introduced in mathematics as distinct peaks, in a poetic way as: "With each modality, a unique part to play,
...
In pursuit of making a AI capable to understand the world, the field of machine learning seeks to develop models capable of processing and 
...
For a deeper understanding of multimodality, please refer to the preceding section of this Unit.
...
How do we evaluate these models? We can check how they perform on these datasets given an image and a question, the task is to answer the question correctly!
...
Standard datasets like MSCOCO might be straightforward for a model to learn due to their distribution, which may not adequately demonstrate a model's capacity to generalize across more challenging or diverse datasets. In response, datasets like [Hateful Memes](https://arxiv.org/abs/2005.04790) are created to address
...
This dataset challenges us to consider if models, despite their impressive results, truly grasp compositional relationships like humans or if they're generalizing data. 
For example, earlier version of Stable Diffusion was not able to clearly count fingers. So, there's still lot of amazing work to be done to get the VLM's to the next stage! 

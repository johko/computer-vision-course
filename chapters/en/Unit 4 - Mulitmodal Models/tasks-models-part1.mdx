...
Image-text retrieval involves classifying or ranking image-text pairs based on their similarity. It typically employs a visual encoder, a text encoder, and a multimodal fusion module to project cross-modal representations into a similarity measure. Visual encoders have evolved from global image features to grid features and bottom-up features, while text encoders have explored BoW, RNN, and CNN architectures. Multimodal fusion techniques range from projecting global features into a common space to finer-grained alignment using attention mechanisms and graph neural networks.
...
Now you know what are the some of popular tasks and models involving Image and Text modalities. But you might be wondering on how to train or fine-tune for the above mentioned tasks. So, let's have a glimpse on training the Vision-Language models.
...
Congratulations! you made till the end. Now on to the next section for more on Vision-Language Pretrained Models.

...
BLIP's architecture consists of four distinct models, and these models share some parameters. The first model is an image encoder transformer that creates the embeddings. Following this are three text encoders: the first is a standalone text encoder for embeddings, trained jointly with the image encoder using contrastive loss. The second text encoder incorporates a cross-attention block to interact with the image encoder's output, trained via an image-text matching loss—a binary classification task. The final text encoder begins with causal self-attention, followed by cross-attention to the image encoder. This one is trained using standard cross-entropy loss for autoregressive language models. The diagram below illustrates BLIP's architecture, highlighting parameter sharing with color-coded blocks.
![BLIP Architecture](https://huggingface.co/datasets/hf-vision/course-assets/resolve/main/BLIP.png)

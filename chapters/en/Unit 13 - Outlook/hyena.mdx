...
Hyena is a new type of operator that substitute to the attention mechanism. Developed by Hazy Research, it features a subquadratic computational efficiency, constructed by interleaving implicitly parametrized long convolutions and data-controlled gating.
...
It consists in recursively computing convolutions and multiplicative element-wise gating operations. This approach builds on top of the H3 mechanism, also developed by the same researchers. The H3 mechanism is characterized by its data-controlled, parametric decomposition, acting as a surrogate attention mechanism.
...
In terms of computational efficiency, the Hyena operator achieves an evaluation time complexity of $O(L \times log_2 L)$ , signifying a significant improvement in processing speed.


### Hyena operator

Let's delve into the second-order recursion of the Hyena operator, which simplifies its representation for illustration purposes.
...
Convolutions can be transformed into matrix multiplication using a Toeplitz matrix. 
...
While Transformer is a well established and very capable architecture, the quadratic computational cost is an expensive price to pay, specially in inference. The H3 mechanism proposition went close to the perplexity of multi-headed attention mechanisms, but there was still a narrow gap in terms of perplexity. 

A variety of attention replacements have been proposed over the last few years, and it remains challenging to evaluate the quality of a new architecture during the exploratory phase. Creating a versatile layer that can effectively process N-Dimensional data within deep neural networks while keeping a good expressiveness is a significant area of ongoing research.

Empirically, Hyena operators are able to significantly shrink the quality gap with attention at scale, reaching similar perplexity and downstream performance with a smaller computational budget and without hybridization of attention. It has already reached State of the art for DNA sequence modeling and is very promising in the field of large language models with Stripped-Hyena-7B. 

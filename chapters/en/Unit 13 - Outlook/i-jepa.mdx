...
- **Invariance-based methods**: In these methods, the model tries to reproduce similar embeddings for different views of the same image. And of course, these different views are hand-crafted, the image augmentations that we’re all familiar with. For example: rotating, scaling, and cropping. These methods are good at producing representations at high semantic levels, but the problem is that they introduce strong biases that may be detrimental to certain downstream tasks. For example, image classification and instance segmentation do not require data augmentations.
...
Now let’s talk about Joint-Embedding Architecture.
...
![I-JEPA Comparisons](https://huggingface.co/datasets/hf-vision/course-assets/resolve/main/i-jepa-1.png?download=true)
...
1. **Target Encoder**: Encodes target images and the target blocks are produced by masking its output.

2. **Context Encoder**: Encodes a randomly sampled context block from the image to obtain a corresponding patch-level representation.
...
![I-JEPA method](https://huggingface.co/datasets/hf-vision/course-assets/resolve/main/i-jepa-2.png?download=true)

## Why It Matters

So, why I-JEPA? I-JEPA introduced many new design features while still being a simple and efficient method for learning semantic image representations without relying on hand-crafted data augmentations. In brief:

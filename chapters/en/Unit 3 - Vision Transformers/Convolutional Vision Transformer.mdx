...
The below table shows the key differences in terms of neccessity of positional encodings, type of token embedding, type of projection, and Transformer structure in the backbone, between the above representative concurrent works and CvT.
...
- Convolutional Transformer block levaraging a **Convolutional Projection**.
...
You can implement CvT for your own usecase by installing HuggingFace `transformers` library as follows:
```bash

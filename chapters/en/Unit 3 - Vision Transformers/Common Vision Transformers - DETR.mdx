...
DETR is mainly used for object detection tasks, which is the process of detecting objects in an image. For example, the input of the model would be an image of a road, and the output of the model could be `[('car',X1,Y1,W1,H1),('pedestrian',X2,Y2,W2,H2)]`, in which X, Y, W, H represent the x, y positions, width and height respectively. <br/><br/>
A traditional object detection model like YOLO consists of hand-crafted features like anchor box priors, which requires initial guesses of object locations and shapes, affecting downstream training. Post-processing steps are then used to remove overlapping bounding boxes, which require careful selection of its filtering heuristics. <br/><br/>
DEtection TRansformer, DETR for short, simplifies the detector by using an encoder-decoder transformer after the feature extraction backbone to directly predict bounding boxes in parallel, requiring minimal post-processing. <br/><br/>
<img src="https://huggingface.co/datasets/hf-vision/course-assets/resolve/main/DETR.png" alt="Architecture Diagram of DETR" width="100%"><br/>
...
<br/><br/><img src="./DETR_FeatureMaps.png" alt="Feature Maps of Encoder" width="100%"><br/>
_**Left**: 12 out of 256 Dimensions in the Feature Map Visualized. Each dimension extract some features of the original cat image, while downsizing the original image. Some dimensions have higher focus on the patterns on the cat, some have a higher focus on the bed sheets. <br>**Right**: Keeping the original feature dimensions, the width and height are further downsized and flattened into size 850._<br/><br/>
Since transformers are permutation invariant, positional embeddings are added to both the encoder and decoder to remind the model where on the image the embeddings come from. In the encoder, fixed positional encodings are used, while in the decoder, learned positional encodings (object queries) are used. 
...
## Evolution of DETR
### Deformable DETR
Two of the main problems of DETR is a long and slow process of convergence, and suboptimal small object detection. <br/>
<br/>**Deformable Attention**<br/>
...
<br/><br/>**Multi-scale Deformable Attention Module**<br/>
...
## References
[1] [DETR](https://arxiv.org/abs/2005.12872) <br/>
[2] [YOLO](https://arxiv.org/abs/1506.02640) <br/>
[3] [YOLOv3](https://arxiv.org/abs/1804.02767) <br/>
[4] [Conditional DETR](https://arxiv.org/abs/2108.06152) <br/>
[5] [Deformable DETR](https://arxiv.org/abs/2010.04159) <br/>
[6] [`facebook/detr-resnet-50`](https://huggingface.co/facebook/detr-resnet-50)<br/>

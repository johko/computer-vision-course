...
```python
!pip install -q natten # Dinat backbone requires Natten library, which may take a while to install.
...
def run_segmentation(image, task_type):
    ...
    processor = OneFormerProcessor.from_pretrained("shi-labs/oneformer_ade20k_dinat_large")  # Load once here 

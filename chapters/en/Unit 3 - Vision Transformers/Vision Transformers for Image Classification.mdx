...
It's not feasible for everyone to train a Vision Transformers on millions of images to get good performance. Instead, one can take advantage of openly available models from places such as the [Hugging Face Hub](https://huggingface.co/models?sort=trending).
...
Since we are taking advantage of the learned features, we do not need to update the entire model either. By freezing the majority of the weights, we can choose to train only certain layers to get excellent performance with less training time and low GPU consumption.

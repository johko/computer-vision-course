...
A digram of the MobileVit Block is shown below:
...
- For each channels these images are unfolded into flattened patches.
...
<Note>
A receptive feild is the size of a region in an input space that affects the features of a particular layer.
</Note>
...
Not, only that the attention method also does not use any batch wise matrix multiplications which helps reduce latency on resource constrained devices like mobile phones.
...
<br><br>
Self-Seperable Attension was also not the first paper to have O(k) complexity. In Linformer,
<br><br>
O(k) complexity for Attension was also achived in Linformer (ADD PAPER LINK HERE) before self seperable attention.
<br><br>
...
In both the the Transformer and Linformer, both of Attention computations preform two batch wise matrix multiplications.  

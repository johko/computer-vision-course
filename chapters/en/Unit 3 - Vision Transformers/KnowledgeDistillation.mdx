...
Presumably, we've all had teachers who "teach" by simply providing us the correct answers and then test us on questions we haven't seen before, analogous to supervised learning
...
Where the KL loss refers to the ![Kullback-Leibler Divergence](https://en.wikipedia.org/wiki/Kullback–Leibler_divergence) between the teacher and the student's output distributions. 
...
To see this loss function implemented in Python as well as a fully worked out example in Python, let's check out the [notebook for this section](../notebooks/Unit 3 - Vision Transformers/KnowledgeDistillation.ipynb).

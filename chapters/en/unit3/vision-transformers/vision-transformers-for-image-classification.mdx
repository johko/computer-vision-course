...
SORRY IF THIS IS THE WRONG PLACE but I am trying to save other folk's time, who might have the same issue I had.
The first Colab doesn´t work, for the 'image' in the dataset isn´t a PIL format, but a dict with key 'bytes'.
The second Colab doesn't work, for when training some errors occur in line 9 of train_transforms.
Thanks and kudos for the awesome HF site.

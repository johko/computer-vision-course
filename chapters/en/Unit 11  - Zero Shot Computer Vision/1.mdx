...
1. **Attribute vectors**: Thinks of attributes as a tabular representation of the different features of the object. 
2. **Textual Descriptions**: A text describing the object contained in the image similar to image captions.
3. **Class label vectors**: Simply by using the embeddings of class labels.
...
But Anyway, In this unit we are going to address *only* embedding-based methods, including **attention-based embedding-based methods**. Zero-shot learning is a big topic and comprehensive coverage may require a course of its own. Interested readers may check the provided additional readings and have fun.

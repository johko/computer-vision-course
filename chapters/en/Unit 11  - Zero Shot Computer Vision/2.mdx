...
Embeddings refers to methods in NLP to represent texts by vectors of real numbers. Any text such as words, phrases, or sentences can be transformed into numerical vectors based on set processes. Traditionally words are represented as one-hot vectors, where each word is independent and has no relationship to one another. Semantic embeddings describe words in a high-dimensional space where the distance and direction between words reflect their semantic relationships. This enables machines to understand the usage, synonyms, and context of each word by mathematical operations on the word embeddings. 
...
- [4] Pourpanah et al., A Review of Generalized Zero-Shot Learning Methods (2022). 

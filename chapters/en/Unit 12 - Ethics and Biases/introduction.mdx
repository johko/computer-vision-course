...
Imagine you wake up on a Sunday morning and play around with your phone. You come across this app which tries to return sarcastic and funny labels if you upload different images or maybe take a selfie. You don't mind some fun, so you try the app out by uploading a selfie and to your shock, it returns an alarming label. It mentions you as a crime suspect (this crime can also be highly dangerous and heinous). You also see social media posts on the same app of different people with provoking labels, increasing the chances of racial and gender profiling. Some of these labels would mean: a person who is a crime offendor, a person having specific facial features that are linked with one's ethnicity, or a person's ancestory. This app returns very offensive labels oftenly and might harm self-interests and target a special group of people. A wide range of such labels which can offend people based on their religion, ethnicity, gender or age were present in the app, and you are just shocked and also kind of confused on what's going on with this? 

![ImageNet Roulette Tweet](https://huggingface.co/datasets/hf-vision/course-assets/blob/main/ethics_bias/IR Tweet.jpg)
...
ImageNet is a large-scale dataset that was created for object recognition benchmarks at scale. The aim was to map out the entire world of objects to make machines around us smarter in scene understanding, in which humans are far better. This dataset was one of the earliest attempts of it's kind in creating a large scale dataset for object recognition. The ImageNet team started scraping image data from various sources on the internet. The original dataset consisted of around 14,197,122 images with 21,841 classes, this was referred to as Imagenet-21K reflecting around 21K classes. The annotations were crowdsourced using Amazon Mechanical Turks. A smaller subset of this dataset which was termed as ImageNet-1K contained 1,281,167 training images, 50,000 validation images and 100,000 test images with 1000 classes which was used as the foundation for the popular ImageNet Large Scale Visual Recognition Challenge (ILSVRC). The ILSVRC became the competition ground for many aspiring companies and labs working in the field of computer vision, to outperform previous approaches on labeling the objects accurately. The structure for ImageNet was based on WordNet, a database of word classifications developed at Princeton University.

![ImageNet General](https://huggingface.co/datasets/hf-vision/course-assets/blob/main/ethics_bias/imagenet.png)
...
The model was trained using Caffe on the images and labels in the "person" category. The app prompted the user for an image, and a face detector detects faces in the image. The detected face was then sent to the Caffe model for classification and finally the app returned an image with bounding box around the detected face and the label the Caffe model predicted. Let us see some example images from the experiment:
*Note: We have blurred faces and labels that might be offensive and triggering to readers so that the readers get an idea in a neutral manner.*

![ImageNet Roulette 1](https://huggingface.co/datasets/hf-vision/course-assets/blob/main/ethics_bias/crime offendor.jpg)
*A person being identified as a first time crime offendor, in the bottom it also shows the word connections from WordNet.*
![ImageNet Roulette 2](https://huggingface.co/datasets/hf-vision/course-assets/blob/main/ethics_bias/ethnic slur.jpg)
*A person being identified on the basis of their ancestory and ethnicity.*
...
![Good person IR](https://huggingface.co/datasets/hf-vision/course-assets/blob/main/ethics_bias/goodperson.jpg)
*This is an example of non-imageable concept. Logically, a person cannot be identified as good depending just on visual appearance.*
...
![Balancing Images](https://huggingface.co/datasets/hf-vision/course-assets/blob/main/ethics_bias/balancing.png)
*This image shows the results before and after balancing the ImageNet database on the basis of gender, skin-color and age.*
...
![Face Obfuscation](https://huggingface.co/datasets/hf-vision/course-assets/blob/main/ethics_bias/crime suspect.jpg)
*A person being identified as a crime suspect (we removed the exact label as the crime was dangerous). Before returning sensitive labels and images, face obfuscation can be used to preserve individual identity without harming it.*
